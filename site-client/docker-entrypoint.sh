...
then
python3 aktin-fhir-mii-report-client.py --fhirurl $FHIR_BASE_URL --brokerurl $BROKER_ENDPOINT_URI --apikey $CLIENT_AUTH_PARAM --fhiruser $FHIR_USER \
...
--sendreport ${OBFUSCAT_COUNT:+--obfuscate}
else
python3 aktin-fhir-mii-report-client.py --fhirurl $FHIR_BASE_URL --brokerurl $BROKER_ENDPOINT_URI --apikey $CLIENT_AUTH_PARAM --fhiruser $FHIR_USER \
--fhirpw $FHIR_PW --fhirtoken $FHIR_TOKEN --httpsproxyaktin $AKTIN_HTTPS_PROXY --httpproxyfhir $FHIR_PROXY_HTTP --httpsproxyfhir $FHIR_PROXY_HTTPS ${OBFUSCATE_COUNT:+--obfuscate}
fi
